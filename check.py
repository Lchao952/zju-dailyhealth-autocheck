    ...
    def take_in(self, geo_info: dict):
        ...
        try:
            ...
            data = {
                ...
                'ismoved': '0',
                'zgfx14rfhsj':'',
                'campus': '工程师学院', # 紫金港校区 玉泉校区 西溪校区 华家池校区 之江校区 海宁校区 舟山校区 宁波校区 工程师学院 杭州国际科创中心 其他 /不在校即为空值
                # 👇-----2022.5.19日修改-----👇
                'verifyCode': ''  ,
                # 👆-----2022.5.19日修改-----👆
                'internship': '1'
    ...
    def Push(self,res):
        if res:
            ...
            if self.DD_BOT_TOKEN:
                ding= dingpush('菲菲每日健康打卡 V3.1 ', res,self.reminders,self.DD_BOT_TOKEN,self.DD_BOT_SECRET)
                ding.SelectAndPush()
            else:
